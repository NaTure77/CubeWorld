			...
			for(int i = 0; i < size.x; i++)
				for(int j = 0; j < size.y; j++)
					for(int k = 0; k < size.z; k++)
					{
						...
						GetPixel(i,j,k).color = ConsoleColor.White;
					}
			int endX = size.x -1;
			int endY = size.y -1;
			int endZ = size.z -1;
			...
			for(int i = 0; i < size.x; i++)
				for(int j = 0; j < size.y; j++)
					for(int k = 0; k < size.z; k++)
					{
						...
						{
							SetPixel(i,j,k,new Pixel(new XYZ_d(i,j,k).Add(toGlobal),3));
							GetPixel(i,j,k).color = ConsoleColor.DarkMagenta;
						}
					}
		}
	}
			...
			for(int i = 0; i < size.x; i++)
				...
				
				{
					if(Math.Pow(i - radius,2) + Math.Pow(k - radius,2) < Math.Pow(radius,2))
					{
						...
						{
							SetPixel(i,j,k,new Pixel(new XYZ_d(i,j,k).Add(toGlobal),3));
							GetPixel(i,j,k).color = ConsoleColor.Yellow;
						}
						if(Math.Pow(i - radius,2) + Math.Pow(k - radius,2) >= Math.Pow(radius-1,2))
						{
							for(int j = 0; j < 1; j++)
							{
								GetPixel(i,j,k).color = ConsoleColor.Black;
								GetPixel(i,heigh-j - 1,k).color = ConsoleColor.Black;
							}
