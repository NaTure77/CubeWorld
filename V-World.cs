...
namespace VirtualCam
{
	class World
	{
        ...
        public void DrawMap(Bitmap img)
        {
            unsafe
            {
                ...
                int heightInPixels = worldSize.y > bitmapData.Height ? bitmapData.Height : worldSize.y;
                int widthInPixels = worldSize.x > bitmapData.Width ? bitmapData.Width : worldSize.x;
                ...
                Parallel.For(0, heightInPixels, y =>
                {
                    ...
                    Parallel.For(0, widthInPixels, x =>
                    {
						...
						Map[x, y, z0].OnRendered = renderer_block;
							...
							Map[x, y, z0].color.Set(currentLine[x * bytesPerPixel + 2],
												 currentLine[x * bytesPerPixel + 1],
												 currentLine[x * bytesPerPixel]);
		...
		public void DrawBuilding(Bitmap img, XYZ startPos)
        {
            unsafe
            {
                BitmapData bitmapData = img.LockBits(new Rectangle(0, 0, img.Width, img.Height), ImageLockMode.ReadWrite, img.PixelFormat);
				  ...
				  Parallel.For(0, heightInPixels, y =>
                {
                    ...
                    Parallel.For(0, widthInPixels, x =>
                    {
						if(currentLine[x * bytesPerPixel] == 200 )
						{
                 ...
                 Parallel.For(0, heightInPixels, y =>
                {
                    ...
                    {
						int z0 = startPos.z - 1;
						if(currentLine[x * bytesPerPixel] < 10)
						{
							Map[x, y, z0].touchable = true;
                            Map[x, y, z0].OnRendered = renderer_block;
							...
							for (int z = z0 - 1; z >= startPos.z - 9; z--)
							{
							   ...
							   Map[x, y, z].color.Set(200, 200, 200);
							}
						}
        ...
        public void DrawCloud(Bitmap img, int height)
        {
            unsafe
            {
                BitmapData bitmapData = img.LockBits(new Rectangle(0, 0, img.Width, img.Height), ImageLockMode.ReadWrite, img.PixelFormat);
                ...
                Parallel.For(0, heightInPixels, y =>
                {
                    ...
                    Parallel.For(0, widthInPixels, x =>
                    {
                        if (currentLine[x * bytesPerPixel] > 0)
                        {
